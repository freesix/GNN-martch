...
def draw_points(img, points, color=(0,255,0),radius=3):
    dp = [(int(points[i, 0]), int(points(i, 1))) for i in range(points.shape[0])] #要画出点的坐标
...
#画出匹配点
#corr1:图A匹配成功的特征点 inlier:真实标签配对点(剔除了阈值以外的配对点后)
def draw_match(img1, img2, corr1, corr2, inlier=[True], color=None, radius1=1, radius2=1, resize=None):
    ...
    corr1_key = [cv2.KeyPoint(corr1[i, 0], corr1[i, 1], radius1) for i in range(corr1.shape[0])] #将匹配点转换为cv2.keypoint类并齐次坐标
    corr2_key = [cv2.KeyPoint(corr2[i, 0], corr2[i, 1], radius2) for i in range(corr2.shape[0])]

    assert len(corr1) == len(corr2)

    draw_matchs = [cv2.DMatch(i, i, 0) for i in range(len(corr1))]
    if color is None:
        color = [(0, 255, 0) if cur_inlier else (0, 0, 255) for cur_inlier in inlier] #标签中有配对成功的
    if len(color) == 1:
        display =cv2.drawMatches(img1, corr1_key, img2, corr2_key, draw_matchs, None,
                                 matchColor=color[0],
        ...
        display[:img2.shape[0], img2.shape[1]:] = img2
