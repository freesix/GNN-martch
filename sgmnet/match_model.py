...
class attention_propagantion(nn.Module):
    ...
    def forward(self,desc1,desc2,weight_v=None):
        ...
        query,key,value=self.query_filter(desc1).view(batch_size,self.head,self.head_dim,-1), self.key_filter(desc2).view(batch_size,
                            self.head, self.head_dim,-1), self.value_filter(desc2).view(batch_size,self.head,self.head_dim,-1)
        if weight_v is not None:
            value = value*weight_v.view(batch_size,1,1,-1)
...
#     values, indices = torch.topk(out,k=separate_num,dim=1,sorted=False)
#     evalu_score=torch.sigmoid(values).squeeze(1)
...
class hybrid_block(nn.Module):
    def __init__(self, channel, head):
        ...
        self.separate_down = Separate_out(2*channel,1)
        self.separate_up = Separate_out(2*channel,1)
        self.attention_block_self=attention_propagantion(channel,head)
        self.attention_block_up=attention_propagantion(channel,head)

    def forward(self,desc1,desc2,seed_index1,seed_index2,separate_num1,separate_num2):
        '''
        seed_index1:   16*128
        desc1:         16*128*1000
        '''
    ...
    def forward(self,data,test_mode=True):
        ...
        for i in range(self.layer_num): # 网络层数
            ...
            if i in self.seedlayer and i!=0: # 在第6次的时候重播种
                ...
                p=sink_algorithm(M,self.mid_ditbin[str(i)],self.sink_iter[seed_para_index-1])
                mid_p_tower.append(p)

                # 重新选取种子
                values,nn_index=torch.topk(p[:,:-1,:-1],k=1,dim=-1) #这里去掉行和列的最后一个因为sink_horn算法增加列和行的长度
